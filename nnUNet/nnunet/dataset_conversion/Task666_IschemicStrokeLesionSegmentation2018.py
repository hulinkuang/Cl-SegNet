...
def get_subfiles(folder, join=True, prefix=None, suffix=None, sort=True):
    ...
    if sort:
        res.sort(key=lambda x: int(prog.match(x).group(1)))
    return res


if __name__ == "__main__":
    train_dir = "/home/wyh/Codes/dataset/ISLES/RawData/ISLES2018_Training/TRAINING"
    test_dir = "/home/wyh/Codes/dataset/ISLES/RawData/ISLES2018_Testing/TESTING"


    output_folder = "/home/wyh/Codes/CoTr_ISLES/nnUNet/nnU_data/nnUNet_raw_data_base/nnUNet_raw_data/Task666_ISLES"
    ...
    def load_save_train(args):
        ...
        pat_id = prog.match(data_file).group(1)
        pat_id = "isl_" + pat_id

        img_itk = sitk.ReadImage(data_file, sitk.sitkFloat32)
    ...
    def load_save_test(args):
        data_file = args
        pat_id = prog.match(data_file).group(1)
        pat_id = "isl_" + str(int(pat_id) + 100)

        img_itk = sitk.ReadImage(data_file)
        sitk.WriteImage(img_itk, join(img_dir_te, pat_id + "_0000.nii.gz"))
        return pat_id

    nii_files_tr_data = get_subfiles(train_dir, True, 'SMIR.Brain.XX.O.CT.', '.nii')
    nii_files_tr_seg = get_subfiles(train_dir, True, 'SMIR.Brain.XX.O.OT.', '.nii')

    nii_files_ts = get_subfiles(test_dir, True, 'SMIR.Brain.XX.O.CT.', '.nii')
    ...
    json_dict['name'] = "ISLES"
    json_dict['description'] = " Ischemic Stroke Lesion Segmentation"
    ...
    splits[-1]['train'] = [i for i in train_ids if int(i[4:]) <=60]
    splits[-1]['val'] = [i for i in train_ids if int(i[4:]) > 60 and int(i[4:]) <= 77]
    splits[-1]['test'] = [i for i in train_ids if int(i[4:]) > 77]
